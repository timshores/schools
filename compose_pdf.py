...
# Uses school_shared.py to keep colors and data prep consistent.
...
from school_shared import (
    OUTPUT_DIR, EXCEL_FILE, load_data, create_or_load_color_map, color_for, norm_label,
    ENROLL_KEYS, EXCLUDE_SUBCATS, DISTRICTS_OF_INTEREST, context_for_district,
    prepare_district_epp_lines, prepare_western_epp_lines, context_for_western,
    LINE_COLORS_DIST, LINE_COLORS_WESTERN, N_THRESHOLD,
)

# ---------------- Tunables for shading legend ----------------
MATERIAL_DELTA_PCTPTS = 0.02
MATERIAL_MIN_LATEST_DOLLARS = 0.0

# ---------------- Simple paths to chart PNGs ----------------
...
def shade_for_delta(delta: float) -> colors.Color | None:
    if delta is None or not (delta == delta):
        return None
    if abs(delta) < 1e-6:
        return None
...
def para_num_auto(text: str) -> Paragraph:
    t = (text or "").strip()
    style = style_num if not t.startswith("-") else style_num_neg
...
def build_page_dicts(df: pd.DataFrame, reg: pd.DataFrame) -> List[dict]:
    ...
    # Baselines for Western Traditional buckets
    ...
    # 1) Western Traditional pages FIRST
    for bucket in ("le_500", "gt_500"):
        ...
        years = epp.index.tolist()
        latest_year = int(epp.index.max()) if not epp.empty else 0

        rows = []
        for sc in reversed(subcats):
            latest_val = float(epp.loc[latest_year, sc]) if (latest_year and sc in epp.columns) else 0.0
            c5 = compute_cagr_last(epp[sc], 5)
            c10 = compute_cagr_last(epp[sc], 10)
            c15 = compute_cagr_last(epp[sc], 15)
        ...
        total = ("Total",
                 f"${float(np.nansum(latest_vals)):,.0f}",
                 fmt_pct(mean_clean([compute_cagr_last(epp[sc], 5) for sc in subcats])),
                 fmt_pct(mean_clean([compute_cagr_last(epp[sc],10) for sc in subcats])),
                 fmt_pct(mean_clean([compute_cagr_last(epp[sc],15) for sc in subcats])))

        # FTE for table (mean for Western pages)
        fte_rows = []
    ...
    # 2) Individual districts (Amherst-Pelham first)
    for dist in ["Amherst-Pelham"] + [d for d in DISTRICTS_OF_INTEREST if d != "Amherst-Pelham"]:
        ...
        for _k, label in ENROLL_KEYS:
            ...
            if s is None or s.empty:
                continue
            r5 = compute_cagr_last(s, 5); r10 = compute_cagr_last(s, 10); r15 = compute_cagr_last(s, 15)
            fte_rows.append(("#000000" if label == "In-District FTE Pupils" else "#AA0000",  # same as LINE_COLORS_DIST
                             label,
        ...
        base_map = {}
        # Build baseline CAGR map from Western pivots
        title_w, epp_w, _ls, _lm = prepare_western_epp_lines(df, reg, bucket)
        if not epp_w.empty:
            for sc in epp_w.columns:
                s = epp_w[sc]
        ...
        pages.append(dict(
            title=(f"Amherst-Pelham Regional: Expenditures Per Pupil vs Enrollment" if dist == "Amherst-Pelham"
    ...
    for idx, p in enumerate(pages):
        ...
        for i, (label, latest, c5, c10, c15, hexcol, latest_num) in enumerate(p["cat_rows"], start=1):
            cat_rows_rl.append(["",
                                ...
                                Paragraph(c5, style_num if not c5.startswith("-") else style_num_neg),
                                Paragraph(c10, style_num if not c10.startswith("-") else style_num_neg),
                                Paragraph(c15, style_num if not c15.startswith("-") else style_num_neg)])
            swatch_rows.append((i, HexColor(hexcol)))

        total = ["", Paragraph(p["cat_total"][0], style_body)] + [
            Paragraph(p["cat_total"][1], style_num),
            Paragraph(p["cat_total"][2], style_num if not p["cat_total"][2].startswith("-") else style_num_neg),
            Paragraph(p["cat_total"][3], style_num if not p["cat_total"][3].startswith("-") else style_num_neg),
            Paragraph(p["cat_total"][4], style_num if not p["cat_total"][4].startswith("-") else style_num_neg),
        ]

        # Legend rows (district pages only)
        legend_rows = []
        ...
        # paint full-cell swatches in column 0
        ...
        # Legend formatting (merge Cat + $; span red/green over CAGR cols)
        if legend_rows:
        ...
        # Relative-to-Western shading with materiality threshold
        if p.get("page_type") == "district":
            base_map = p.get("baseline_map", {})
            for row_idx, row_raw in enumerate(p["cat_rows"], start=1):
                subcat = row_raw[0]
                latest_num = float(row_raw[6]) if len(row_raw) > 6 else 0.0
